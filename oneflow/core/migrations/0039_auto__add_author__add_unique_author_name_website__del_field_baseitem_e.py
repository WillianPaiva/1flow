...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        db.create_table(u'core_author', (
            ...
            ('origin_name', self.gf('django.db.models.fields.CharField')(max_length=255, blank=True)),
            ('is_unsure', self.gf('django.db.models.fields.BooleanField')(default=False)),
            ('user', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['base.User'], null=True, blank=True)),
            ('duplicate_of', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['core.Author'])),
        ))
        db.send_create_signal('core', ['Author'])

        # Adding unique constraint on 'Author', fields ['name', 'website']
        db.create_unique(u'core_author', ['name', 'website_id'])

        # Deleting field 'BaseItem.excerpt'
        db.delete_column(u'core_baseitem', 'excerpt')

        # Deleting field 'BaseItem.image_url'
        db.delete_column(u'core_baseitem', 'image_url')

        # Adding M2M table for field authors on 'BaseItem'
        m2m_table_name = db.shorten_name(u'core_baseitem_authors')
        ...
        db.create_unique(m2m_table_name, ['baseitem_id', 'author_id'])

        # Deleting field 'Article.pages_urls'
        db.delete_column(u'core_article', 'pages_urls')

        # Adding field 'Article.image_url'
        db.add_column(u'core_article', 'image_url',
                      self.gf('django.db.models.fields.URLField')(max_length=200, null=True, blank=True),
                      keep_default=False)

        # Adding field 'Article.excerpt'
        db.add_column(u'core_article', 'excerpt',
                      self.gf('django.db.models.fields.TextField')(null=True, blank=True),
                      keep_default=False)


    def backwards(self, orm):
        # Removing unique constraint on 'Author', fields ['name', 'website']
        db.delete_unique(u'core_author', ['name', 'website_id'])

        # Deleting model 'Author'
        db.delete_table(u'core_author')


        # User chose to not deal with backwards NULL issues for 'BaseItem.excerpt'
        raise RuntimeError("Cannot reverse this migration. 'BaseItem.excerpt' and its values cannot be restored.")
        
        # The following code is provided here to aid in writing a correct migration        # Adding field 'BaseItem.excerpt'
        db.add_column(u'core_baseitem', 'excerpt',
                      self.gf('django.db.models.fields.TextField')(),
                      keep_default=False)


        # User chose to not deal with backwards NULL issues for 'BaseItem.image_url'
        raise RuntimeError("Cannot reverse this migration. 'BaseItem.image_url' and its values cannot be restored.")
        
        # The following code is provided here to aid in writing a correct migration        # Adding field 'BaseItem.image_url'
        db.add_column(u'core_baseitem', 'image_url',
                      self.gf('django.db.models.fields.URLField')(max_length=200),
                      keep_default=False)

        # Removing M2M table for field authors on 'BaseItem'
        db.delete_table(db.shorten_name(u'core_baseitem_authors'))

        # Adding field 'Article.pages_urls'
        db.add_column(u'core_article', 'pages_urls',
                      self.gf('jsonfield.fields.JSONField')(default=u'[]', blank=True),
                      keep_default=False)

        # Deleting field 'Article.image_url'
        db.delete_column(u'core_article', 'image_url')

        # Deleting field 'Article.excerpt'
        db.delete_column(u'core_article', 'excerpt')
    ...
    models = {
        ...
        u'base.user': {
            ...
            'hash_codes': ('jsonfield.fields.JSONField', [], {'default': "{'unsubscribe': 'a786d93dd67944e687832f5be288b90d'}", 'blank': 'True'}),
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
        ...
        'core.author': {
            ...
            'duplicate_of': ('django.db.models.fields.related.ForeignKey', [], {'to': "orm['core.Author']"}),
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
        ...
        'core.basefeed': {
            ...
            'errors': ('jsonfield.fields.JSONField', [], {'default': '[]', 'blank': 'True'}),
